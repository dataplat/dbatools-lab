...
\s+Description = "Container for AG tests"' ,'' -replace 'D:\\temp','/tmp' -replace ([Regex]::Escape('\mssql1.AdventureWorks.DataMaskingConfig.json')), '/localhost.15592.AdventureWorks2017.DataMaskingConfig.json' -replace 'PostalCode -Path /tmp','PostalCode -Path /tmp |Select FullName | OUt-Host;dir /tmp | Out-Host'
