...
\s+Description = "Container for AG tests"' ,'' -replace 'D:\\temp','/tmp' -replace ([Regex]::Escape('\mssql1.AdventureWorks.DataMaskingConfig.json')), '/localhost.15592.AdventureWorks2017.DataMaskingConfig.json' -replace 'PostalCode -Path /tmp','PostalCode -Path /tmp |Select $_.FullName | OUt-Host;dir /tmp | Out-Host'
...
Describe "Checking the file <_.Name> code works as intended" -ForEach $files[19] {
    $filename = $_.Name
