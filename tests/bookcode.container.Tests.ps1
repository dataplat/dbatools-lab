...
Import-DbaSpConfigure @splatExportSpConf -WarningAction SilentlyContinue' -replace 'Source\s+= "dbatoolslab"','Source      = "localhost,15592"' -replace ([Regex]::Escape('Destination\s+= "dbatoolslab"')),'Destination      = "localhost,15593"' -replace ([Regex]::Escape('SharedPath  = "\\dbatoolslab\Backup"')),'SharedPath  = "/var/opt/mssql/backups/shared"' -replace ([Regex]::Escape('SharedPath    = "C:\dbatoolslab\Backup"')),'SharedPath  = "/var/opt/mssql/backups/shared"'
