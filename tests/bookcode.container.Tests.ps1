...
' -replace ([Regex]::Escape('Export-DbaInstance -SqlInstance ''localhost,15592'' -Path /tmp/backups/container')),'Export-DbaInstance -SqlInstance ''localhost,15592'' -Path /tmp/backups/container -Exclude Credentials' -replace ([Regex]::Escape('Exclude = "ResourceGovernor"')),'Exclude = "ResourceGovernor,Credentials"'
