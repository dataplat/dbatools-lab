...
\s+OutVariable = file', 'Path = "/tmp/"
  ...
  Force = $true' -replace 'Schedule = ''WorkingWeek-Every-3-Hours''', 'Schedule = ''WorkingWeek-Every-15-Minute''' -replace 'Database = ''FactorySales''' , 'Database = ''tempdb'''
...
Describe "Checking the file <_.Name> code works as intended" -ForEach $files[0..18] {
    $filename = $_.Name
