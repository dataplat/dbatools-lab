...
PS> Backup-DbaDatabase @backupparam', 'Backup-DbaDatabase @backupparam' -replace ([Regex]::Escape('-Password $pw')),'-Password $sqlcred'  -replace ([Regex]::Escape('$pw = (Get-Credential wejustneedthepassword).Password')),'#' 
